...
//  newBoostHTTP
//
//  Created by Israel Junior on 06/03/15.
...
struct hello_world {
    ...
    void operator()(server::request const &request, server::response &response) {
        ...
        server::string_type metodo = request.method;
        std::ostringstream data;
        data << "Hello, " << ip << ':' << port << '!';
        data << "Recebi o metodo : " << metodo;
...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        /*<< Runs the server. >>*/
        std::cout << "Server is run at : http://localhost:80" << std::endl;
        server_.run();
    }
